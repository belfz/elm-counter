...
Elm.Counter.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var cssHighest = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "color",_1: "red"}]));
   var cssCount = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "color",_1: "green"}]));
   var update = F2(function (action,state) {
      var _p0 = action;
      switch (_p0.ctor)
      {case "Increment": return _U.update(state,{count: state.count + 1});
         case "Decrement": return _U.update(state,{count: state.count - 1});
         case "Reset": return _U.update(state,{highest: _U.cmp(state.count,state.highest) > 0 ? state.count : state.highest,count: 0});
         default: return _U.update(state,{count: state.count + _p0._0});}
   });
   ...
   var view = F2(function (address,state) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.div,_U.list([cssCount]),_U.list([$Html.text(A2($Basics._op["++"],"count: ",$Basics.toString(state.count)))]))
              ,A2($Html.div,_U.list([cssHighest]),_U.list([$Html.text(A2($Basics._op["++"],"highest: ",$Basics.toString(state.highest)))]))
              ...
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Reset)]),_U.list([$Html.text("R")]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Add(5))]),_U.list([$Html.text("5")]))]));
   });
   var initialState = {count: 0,highest: 0};
   var State = F2(function (a,b) {    return {count: a,highest: b};});
   return _elm.Counter.values = {_op: _op,initialState: initialState,view: view,update: update};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var main = $StartApp$Simple.start({model: $Counter.initialState,view: $Counter.view,update: $Counter.update});
